...
import { localizeProject, localizeCategory } from "@/lib/i18n/localize";

export function FeaturedProjects() {
  const { t, locale } = useLanguage();
  const featured = projects.filter((p) => p.featured).map((p) => localizeProject(p, locale));
    ...
    <section className="px-6 py-24 md:px-12 md:py-40" aria-label="Featured projects">
      <div className="flex flex-wrap items-end justify-between gap-10">
        <SectionHeading
          ...
          lines={[t("home.featuredProjects.line1"), t("home.featuredProjects.line2")]}
        />
        <FadeUp delay={0.2}>
